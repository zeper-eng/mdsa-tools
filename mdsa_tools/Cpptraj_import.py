...
It parses hbond ... out <file> series output tables, extracts residue–residue hydrogen bond pairs from the header, and loads the corresponding time series into adjacency matrix representations. 
These system-level matrices can then be used directly for downstream analyses (e.g., clustering, dimensionality reduction, visualization).
...
See Also
--------
mdsa_tools.Viz.visualize_reduction — Plot PCA/UMAP embeddings of reduced systems.
mdsa_tools.Data_gen_hbond.create_system_representations — Build system-level adjacency matrices from trajectories.
numpy.linalg.svd — Linear algebra backend for reduction methods.
